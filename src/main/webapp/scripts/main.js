(function () {
    ...
    function onLoadPositionFailed() {
        ...
        getLocationFromIP(); // try to retrieve the location from ip
    }
    ...
    function login() {
        ...
        password = md5(username + md5(password)); // hashes the password, need to add salt
        // The request parameters
        var url = './login';
        var req = JSON.stringify({
            user_id: username,
        ...
        ajax('POST', url, req,
            ...
            function (res) {
                ...
                if (result.status === 'OK') {
                    ...
                    // might not actually exist, but we're saying that anyways for all invalid stuff
    ...
    // Helper Functions
    /**
     * A helper function that makes a navigation button active (a.k.a noticable)
     */
    ...
    function changeFavoriteItem(item) {
        ...
        var method = favorite ? 'POST' : 'DELETE'; // either delete from or add to favorite
        ajax(method, url, req,
            // successful callback
            function (res) {
                var result = JSON.parse(res);
    ...
    function addItem(itemList, item) {
        ...
        favLink.onclick = function () {
            changeFavoriteItem(item); // delete or add to fav
        };
